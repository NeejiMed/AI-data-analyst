...
def compute_revenue_kpis(
    ...
    # refunds
    refunded_count = db.query(func.count(Order.id)).filter(
        base_filter,
    ...
    cost_result = db.query(
        func.sum(Product.cost_price * OrderItem.quantity).label("total_cost")
    ...
    return RevenueKPIs(
        ...
        avg_order_value= round(total_revenue / completed.orders if completed.orders else 0, 2),
        total_customers= completed.customers or 0,
        gross_profit= round(gross_profit, 2),
        gross_margin_pct= round((gross_profit / total_revenue * 100) if total_revenue else 0, 2),
        refund_rate_pct= round((refunded_count / total_orders * 100) if total_orders else 0, 2),
...
def compute_regional_kpis(
    ...
    return [
        RegionalKPIs(
            ...
            market_share_pct=round((float(r.revenue or 0) / total_revenue * 100) if total_revenue else 0, 2)
