...
def compute_rfm_segments(
    db: Session, reference_date: datetime
    ...
    df["r_score"] = pd.qcut(df["recency_days"], q=4, labels=[4, 3, 2, 1]).astype(
        int
    )  # more recent = higher score
    df["f_score"] = pd.qcut(
        df["frequency"].rank(method="first"), q=4, labels=[1, 2, 3, 4]
    ).astype(int)  # more frequent = higher score
    df["m_score"] = pd.qcut(
        df["monetary"].rank(method="first"), q=4, labels=[1, 2, 3, 4]
    ).astype(int)  # higher spend = higher score
    df["rfm_score"] = (
        df["r_score"] + df["f_score"] + df["m_score"]
    )  # the higher the better ranking, ranges from 3 to 12
