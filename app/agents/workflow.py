...
class AnalyticsWorkflow:
    ...
    def _run_sales_trend_pipeline(
        ...
        # Analytics
        result = self.analytics.analyze_sales_trends(
        ...
        chart_paths = {
            name: data["paths"] for name, data in charts.items()
        }
    ...
    def _run_segmentation_pipeline(
        ...
        report = self.report_builder.build_segmentation_report(
            result, insights
        )
        md_path = self.md_renderer.save(report)
    ...
    def _run_anomaly_pipeline(
        ...
        messages = build_anomaly_explanation_prompt(
            anomaly_lines, trend_lines
        )

        explanation = parse_text_response(
            call_llm(messages, temperature=0.3)
        )
