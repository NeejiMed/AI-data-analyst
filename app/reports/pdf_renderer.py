...
class PDFRenderer:
    ...
    def save(self, report: Report, output_dir: str = "outputs/reports") -> str:
        ...
        if not _check_reportlab():
            logger.warning("reportlab_not_installed_falling_back_to_markdown")
        ...
        for section in sorted(report.sections, key=lambda s: s.order):
            ...
            if section.type in (SectionType.SUMMARY, SectionType.NARRATIVE):
                ...
                if section.content:
                    ...
                    data = [headers] + [
                        [row.get(h, "") for h in headers] for row in section.content
                    ]
                ...
                for insight in section.content:
                    ...
                    elements.append(
                        Paragraph(insight.get("explanation", ""), style_body)
                    )
                    elements.append(
                        Paragraph(
                ...
                for i, action in enumerate(section.content, 1):
                    elements.append(Paragraph(f"{i}. {action}", style_body))
