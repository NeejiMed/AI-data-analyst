...
class ReportSection:
    type: (
        SectionType  # The type of section, which determines how it should be rendered.
    )
    title: str  # A title for the section, which can be used in the report's table of contents or as a header.
    content: (
        str | list | dict
    )  # Depending on the section type, content can be a string (for narrative sections), a list of insights, or a dict for tables.
...
class Report:
    ...
    )  # A list of sections that make up the report.
    metadata: dict = field(
        default_factory=dict
    )  # Additional metadata about the report, such as the data sources used, the analyst who generated it, etc.

    def add_section(self, section: ReportSection) -> None:
        """Add a section to the report."""
        section.order = len(
            self.sections
        )  # Set the order based on the current number of sections
        self.sections.append(section)

    @property
    def filename_base(self) -> str:
        """Generate a base filename for the report based on its title and period."""
        date_str = self.generated_at.strftime(
            "%Y%m%d_%H%M%S"
        )  # Use generation timestamp for uniqueness
