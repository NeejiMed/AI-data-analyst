...
class InsightPoint(BaseModel):
    ...
    recommendation: str = Field(description="Actionable recommendation for the business")

class AnalyticsInsights(BaseModel):
    executive_summary: str = Field(description="3-4 sentence summary for a business executive")
    key_insights: list[InsightPoint] = Field(description="3-5 specific insights from the data")
    ...
    recommended_actions: list[str] = Field(description="Top 3 recommended actions for the business")

class AnomalyExplanation(BaseModel):
    period: str
    what_happened: str = Field(description="Plain english description of the anomaly")
    likely_cause: str = Field(description="Most probable business reason for the anomaly")
